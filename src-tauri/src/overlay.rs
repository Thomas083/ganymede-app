...
fn update_click_through_state<R: Runtime>(app: &AppHandle<R>, state: &OverlayState) -> Duration {
    ...
    // Contract: an empty interactive region list means no overlay area is interactive,
    // so the whole overlay must stay click-through.
    let should_click_through = snapshot.interactive_regions.is_empty()
